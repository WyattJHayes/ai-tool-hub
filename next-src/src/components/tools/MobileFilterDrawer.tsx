...
export function MobileFilterDrawer(props: MobileFilterDrawerProps) {
  ...
  return (
    <dialog ref={ref} onClose={props.onClose} aria-labelledby="mobile-filter-title" className="m-0 ml-auto h-full max-h-none w-[min(360px,92vw)] border-l border-[var(--line-strong)] bg-[var(--surface)] p-0 backdrop:bg-black/40 lg:hidden">
      ...
      <div className="p-4">
        {!props.state.sceneId ? <label className="mb-6 block text-sm font-semibold">分类<select aria-label="选择分类" value={props.state.categoryId || ''} onChange={(event) => props.onPatch({ categoryId: event.target.value || null })} className="mt-2 min-h-11 w-full rounded-md border border-[var(--line-strong)] bg-[var(--surface)] px-3 text-sm font-normal text-[var(--ink)]"><option value="">全部分类</option>{props.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label> : null}
