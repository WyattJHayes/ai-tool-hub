...
export function FilterFields({ state, platformOptions, radioGroupName, onPatch, onClear }: FilterFieldsProps) {
  return (
    <div className="space-y-6">
      <fieldset><legend className="mb-2 text-sm font-semibold">价格</legend>{prices.map((option) => <label key={option.value || 'all'} className="flex min-h-11 items-center gap-2 text-sm"><input type="radio" name={radioGroupName} checked={state.price === option.value} onChange={() => onPatch({ price: option.value })} className="h-4 w-4 border border-[var(--line-strong)] accent-[var(--accent)]" />{option.label}</label>)}</fieldset>
      <fieldset><legend className="mb-2 text-sm font-semibold">来源</legend>{origins.map((option) => <label key={option.value} className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" checked={state.origins.includes(option.value)} onChange={() => onPatch((currentState) => ({ origins: toggleValue(currentState.origins, option.value) }))} className="h-4 w-4 border border-[var(--line-strong)] accent-[var(--accent)]" />{option.label}</label>)}</fieldset>
      {platformOptions.length ? <fieldset><legend className="mb-2 text-sm font-semibold">平台</legend>{platformOptions.map((value) => <label key={value} className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" checked={state.platforms.includes(value)} onChange={() => onPatch((currentState) => ({ platforms: toggleValue(currentState.platforms, value) }))} className="h-4 w-4 border border-[var(--line-strong)] accent-[var(--accent)]" />{platformLabels[value]}</label>)}</fieldset> : null}
      <button type="button" onClick={onClear} className="min-h-11 w-full rounded-md border border-[var(--line-strong)] px-3 text-sm text-[var(--muted)]">清除筛选</button>
    </div>
  );
}
