  ...
  return (
    <section aria-label="目录条件" className="border-y border-[var(--line)] bg-[var(--surface)] py-0 sm:py-4">
      ...
      <div data-directory-controls className="mt-3 grid min-w-0 grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)] gap-2 lg:grid-cols-[minmax(180px,1fr)_minmax(180px,1fr)_minmax(160px,.7fr)]">
        <label className="min-w-0 max-w-full overflow-hidden text-xs text-[var(--muted)]"><span className="sr-only lg:not-sr-only">任务</span><select aria-label="选择任务" value={state.sceneId || ''} onChange={(event) => onPatch({ sceneId: event.target.value || null })} className="mt-1 min-h-11 w-full min-w-0 max-w-full rounded-md border border-[var(--line)] bg-[var(--surface)] px-2 text-sm text-[var(--ink)] sm:px-3"><option value="">全部任务</option>{scenes.map((scene) => <option key={scene.id} value={scene.id}>{scene.name.replace(/^我要/, '')}</option>)}</select></label>
