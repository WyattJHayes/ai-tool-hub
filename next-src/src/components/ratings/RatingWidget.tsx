...
export function RatingWidget({ toolId, currentRating = 0, onRated }: RatingWidgetProps) {
  ...
  if (submitted) {
    return <div className="flex min-h-[120px] flex-col items-center justify-center gap-2 rounded-lg border border-[var(--accent-soft)] bg-[var(--accent-soft)] p-5 text-center"><Star className="h-5 w-5 fill-current text-[var(--accent)]" /><p className="text-sm text-[var(--accent-ink)]">感谢评价</p></div>;
  }
      ...
      <div className="flex flex-wrap items-center justify-center gap-1">
        {[1, 2, 3, 4, 5].map((value) => (
          <button type="button" key={value} onMouseEnter={() => setHoverScore(value)} onMouseLeave={() => setHoverScore(0)} onClick={() => setScore(value)} className="flex h-11 w-11 items-center justify-center rounded-md hover:bg-[var(--surface-subtle)]" aria-label={`${value} 星`}>
            <Star className={cn('h-7 w-7', value <= displayScore ? 'fill-[var(--accent)] text-[var(--accent)]' : 'text-[var(--line-strong)]')} />
          </button>
        ))}
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {RATING_TAGS.map((tag) => {
              ...
              return <button type="button" key={tag} onClick={() => toggleTag(tag)} aria-pressed={selected} className={cn('min-h-11 rounded-md border px-3 text-xs', selected ? 'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent-ink)]' : 'border-[var(--line-strong)] text-[var(--muted)] hover:bg-[var(--surface-hover)]')}>{tag}</button>;
            })}
          </div>
          <div>
          ...
          {error ? <p role="alert" className="rounded-md border border-[var(--signal-ink)] bg-[var(--signal-soft)] px-3 py-2 text-center text-xs text-[var(--signal-ink)]">{error}</p> : null}
          <button type="button" onClick={handleSubmit} disabled={submitting} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-md bg-[var(--accent)] text-sm font-medium text-[var(--on-accent)] hover:bg-[var(--accent-hover)] disabled:opacity-50"><Send className="h-4 w-4" />{submitting ? '提交中…' : '提交评价'}</button>
        </div>
      ) : null}
    </div>
  );
